export const SERVER_URL =
  process.env.SERVER_URL ?? process.env.NEXT_PUBLIC_SERVER_URL;
export const API_URL = process.env.REST_API_URL ?? process.env.NEXT_PUBLIC_REST_API_URL;
export const API_KEY = process.env.REST_API_KEY ?? process.env.NEXT_PUBLIC_REST_API_KEY;
export const LOCAL_URL = "http://localhost:3000";
export const STRIPE_PUBLIC_KEY = process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY;
export const STRIPE_SECRET_KEY = process.env.STRIPE_SECRET_KEY;
export const SENDCLOUD_PUBLIC_KEY = process.env.SENDCLOUD_PUBLIC_KEY;
export const SENDCLOUD_SECRET_KEY = process.env.SENDCLOUD_SECRET_KEY;
